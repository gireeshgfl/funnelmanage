...
def error_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            # Log object data and arguments
            if args:
                instance = args[0]  # Typically 'self'
                if hasattr(instance, '__dict__'):
                    logger.debug(f"Object (self) data: {instance.__dict__}")
                else:
                    logger.debug(f"Object (self): {instance}")
            logger.debug(f"Arguments: {args[1:]}, Keyword Arguments: {kwargs}")
