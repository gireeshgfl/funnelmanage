...
export default function LoginPage() {
  ...
  const handleRoleBasedRedirect = (role) => {
    ...
    if (role && validRoles.includes(role.toLowerCase())) {
      router.replace(`/funnel-management/funnel-management/dashboard/${role.toLowerCase()}`);
    } else {
      router.replace('/funnel-management/funnel-management/unauthorized');
    }
  };
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 ${isLoading ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {isLoading ? (
                <>
